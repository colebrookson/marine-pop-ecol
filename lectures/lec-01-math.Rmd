...
If we represent $\lambda$ as $2.7183^r$, we now have a lot more of formal mathematical tools at our disposal since that number is actually Euler's constant ($e$). It has the important mathematical property that its natural logarithm is equal to 1. ($ln(e) = 1$).
...
date: march 25, april 01, 08, 15, 22
aphids: 0.02, 0.5, 1.5, 5, 14.5
ln(aphids): -3.91, -0.69, 0.4, 1.61, 2.67
...
So we can actually rewrite our equation here as $$\frac{dN}{dt} = bF_TN\left(\frac{\frac{F_T}{c} - N}{\frac{F_T}{c}} \right)$$, and if we sub back in $r$ for $bF_T$ and $K$ for $F_T/c$ then we get $$\frac{dN}{dt} = rN\frac{K-N}{K}$$ which is the classic form of the logistic equation. has a very simple biological interpretation. 